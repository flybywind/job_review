...
func NewBloomFilter(size int, tol float32, seed int64) *BloomFilter {
	...
	for i := range bf.filters {
		bf.filters[i] = hash.NewMurMurHashWithSeed(bf.rndGen.Int63())
	}
	return &bf
}
...
func minimalKSum2(nums []int, k int) int64 {
	// try bloom filter
	bf := NewBloomFilter(len(nums), 0.01, 0xB34FA7C3)
	hashMap := map[int64]bool{}
	for _, n := range nums {
		bf.Set(int64(n))
		hashMap[int64(n)] = true
	}

	sumn := int64(0)
	for i, n := int64(1), 0; n < k; i++ {
		if bf.Hit(i) && hashMap[i] {
			continue
		}
		sumn += i
		n++
